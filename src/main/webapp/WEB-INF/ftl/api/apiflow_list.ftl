    ...
    <script type="text/javascript">
		...
		if(startTime ==''||endTime==''){
			...
			$("#apiFlowList").jqGrid('setGridParam', {
				url : "${request.contextPath}/apiFlow/queryApiFlowList",
				datatype : "json",
				mtype : "POST",
				postData : getParams()
			}).trigger("reloadGrid");
...
/* 		function formatLink(cellvalue, options, rowObject) {
		...
		function initGrid() {
			$("#apiFlowList").jqGrid({
				...
				datatype : "json",
				mtype : "POST",
				colNames : ['查询日期','查询时刻','请求响应','供应商','接口类型','用户ip','所属 地区',"信息",'traceNo'],
				colModel : [ {
					name : 'date',
					index : 'date',
					align : 'center',
	...
	<div class="breadnav"><span>首页</span> > 流量明细</div>
	 <form id="myForm">
		<div class="infor1">
				...
				<div class="main">
					<div class="part">
						...
						<span>起止时间：</span><input type="text" value="${startTime}" id="startTime" onClick="new Calendar().show(this);" readonly/>
						 - <input type="text" value="${endTime}" id="endTime" onClick="new Calendar().show(this);" readonly/>
						接口类型：
			...
			message：<input type="text" value="" id="message" length="300px" />
			traceNo：<input type="text" value="" id="traceNo" length="300px" />
