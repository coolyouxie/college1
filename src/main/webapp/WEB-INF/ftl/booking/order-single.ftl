...
<form id="myform">  
	...
	<div class="container">
		...
		<#include "booking/bookingFlightInfo.ftl">
		 </div>
		 <div class="passenger">
...
<input id="bookingCount" type="hidden" value="1">
<!--下拉选项框  -->
<div>
...
function getPassengerBirth(i) {
	...
	if(passengerIDCardType=="ID"){
		var passengerBirth = getBirthdatByIdNo(passengerIDCardNo, i);
		//根据身份证获取乘客生日日期
...
//当其他证件类型时候，选取生日区分乘客类型
 ...
 //根据身份证号获取出生日期
 function getBirthdatByIdNo(iIdNo, i) {
		var tmpStr = "";
		var idDate = "";
		var tmpInt = 0;
		var strReturn = "";
...
function checkAtrrPassenger(pid){
	...
	var flightDepartureDate= $("#flightDepartureDate").val();
	if(certType=="ID"){
		var passengerBirth = getBirthdatByIdNo(certNo, pid);
		//根据身份证获取乘客生日日期
				         ...
				         var passengerTdIds = $('#passengers [id^=passengerTdIds]');
				       	 ...
				       	 $.each(passengerIDCardTypes,function(i){
							  if(passengerIDCardTypes[i].value=="ID"){
							       if(getBirthdatByIdNo($(passengerIDCardNos[i]).val(),$(passengerTdIds[i]).text())!=$(passengerBirthdays[i]).val()){
